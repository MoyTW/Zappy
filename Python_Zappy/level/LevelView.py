...
class LevelView(object):
    ...
    def player_actor(self):
        return self._lvl.player_actor

    def get_display_images_at(self, x, y, _in_fow=False):
        if self._lvl.are_valid_coords(x, y):
            return self._lvl._cells[x][y].get_display_images(_in_fow)
        return None

    def get_cell_at(self, x, y):
        if self._lvl.are_valid_coords(x, y):
            return self._lvl._cells[x][y]
        return None

    def cell_is_passable(self, x, y):
        if self._lvl.are_valid_coords(x, y):
            return self._lvl._cells[x][y].is_passable
        return False

    def cell_is_transparent(self, x, y):
        if self._lvl.are_valid_coords(x, y):
            return self._lvl._cells[x][y].is_transparent
        return False
    ...
    def get_all_entities(self):
        entities = list()
        for x in range(self._lvl.level_width):
            for y in range(self._lvl.level_height):
                entities.extend(self.get_all_entities_at(x, y))
        return entities

    def get_all_entities_at(self, x, y):
        if self._lvl.are_valid_coords(x, y):
            return self._lvl._cells[x][y].get_all_entities()
        return None
    ...
    def find_coordinates_of_entity(self, entity):
        for x in range(self._lvl.level_width):
            for y in range(self._lvl.level_height):
                if self._lvl._cells[x][y].contains_entity(entity):
                    return [x, y]
        return None