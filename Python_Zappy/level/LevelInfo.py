...
class LevelInfo:
    ...
    def __init__(self, _name, _number, _width, _height, _previews_folder):
        ...
        self.level_number = _number
        self.level_width = _width
        self.level_height = _height
        self._previews_folder = _previews_folder

        self.preview_image = self._return_preview_image()
    ...
    def _return_preview_image(self):
        path = "{0}/preview_images/{1}.png".format(self._previews_folder, self.level_number)

        try:
            return self.PREVIEW_LOADER.image(path)
        except pyglet.resource.ResourceNotFoundException:
            warnstr = "There is no preview available for level {0}".format(self.level_number)
